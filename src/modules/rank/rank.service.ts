  ...
  async getAll(paginateDto: PaginateDto): Promise<PaginatedResult<Rank>> {
    try {
          ...
          if (key === 'requiredPoints') {
            if (typeof value === 'object' && value !== null) {
              ...
              if (min !== undefined && max !== undefined) {
                filterConditions[key as keyof Rank] = { [Op.between]: [min, max] };
      ...
      const result = await this.rankModel.findAll({
        attributes: ['id', 'name', 'requiredPoints', 'amount' , 'fixedPoints', 'percentagePoints', 'maxPercentagePoints',  'createdAt', 'updatedAt'],
        where: filterConditions,
